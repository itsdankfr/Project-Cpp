...
void move(string str)
{
    ...
    for (int i = 0; i < 32; i++) {
        if (isWhitePawn(f[i]) && int(f[i].getPosition().y / squareSize) == 0) { // check white pawn can promote
            
            RenderWindow window(VideoMode(504, 504), "DJT");
        }

        else if (isBlackPawn(f[i]) && int(f[i].getPosition().y / squareSize) == 7) { // check black pawn can promote
            RenderWindow window(VideoMode(504, 504), "DJT");
        }
